...
# Installation
When on *Raspberry Pi* / Linux, try installing the dependencies with [{bspm}](https://github.com/Enchufa2/bspm) first. This will install the packages as binaries instead of compiling them from source, speeding up the installation of {smcounts} later.
...
Or use `collect_data` to get all platforms (with default arguments to the individual functions):
```{r}
all <- collect_data()
```
...
### helper functions
You can use `mailchimp_lists` to find out the id of the audience you want to track:
...
- for slack: a `.slackr` file, see [https://github.com/hrbrmstr/slackr](https://github.com/hrbrmstr/slackr).
